<?php

if ( ! function_exists( 'playerx_core_map_portfolio_settings_meta' ) ) {
	function playerx_core_map_portfolio_settings_meta() {
		$meta_box = playerx_edge_create_meta_box( array(
			'scope' => 'portfolio-item',
			'title' => esc_html__( 'Portfolio Settings', 'playerx-core' ),
			'name'  => 'portfolio_settings_meta_box'
		) );
		
		playerx_edge_create_meta_box_field( array(
			'name'        => 'edgtf_portfolio_single_template_meta',
			'type'        => 'select',
			'label'       => esc_html__( 'Portfolio Type', 'playerx-core' ),
			'description' => esc_html__( 'Choose a default type for Single Project pages', 'playerx-core' ),
			'parent'      => $meta_box,
			'options'     => array(
				''                  => esc_html__( 'Default', 'playerx-core' ),
				'huge-images'       => esc_html__( 'Portfolio Full Width Images', 'playerx-core' ),
				'images'            => esc_html__( 'Portfolio Images', 'playerx-core' ),
				'small-images'      => esc_html__( 'Portfolio Small Images', 'playerx-core' ),
				'slider'            => esc_html__( 'Portfolio Slider', 'playerx-core' ),
				'small-slider'      => esc_html__( 'Portfolio Small Slider', 'playerx-core' ),
				'gallery'           => esc_html__( 'Portfolio Gallery', 'playerx-core' ),
				'small-gallery'     => esc_html__( 'Portfolio Small Gallery', 'playerx-core' ),
				'masonry'           => esc_html__( 'Portfolio Masonry', 'playerx-core' ),
				'small-masonry'     => esc_html__( 'Portfolio Small Masonry', 'playerx-core' ),
				'custom'            => esc_html__( 'Portfolio Custom', 'playerx-core' ),
				'full-width-custom' => esc_html__( 'Portfolio Full Width Custom', 'playerx-core' )
			)
		) );
		
		/***************** Gallery Layout *****************/
		
		$gallery_type_meta_container = playerx_edge_add_admin_container(
			array(
				'parent'          => $meta_box,
				'name'            => 'edgtf_gallery_type_meta_container',
				'dependency' => array(
					'show' => array(
						'edgtf_portfolio_single_template_meta'  => array(
							'gallery',
							'small-gallery'
						)
					)
				)
			)
		);
		
		playerx_edge_create_meta_box_field(
			array(
				'name'          => 'edgtf_portfolio_single_gallery_columns_number_meta',
				'type'          => 'select',
				'label'         => esc_html__( 'Number of Columns', 'playerx-core' ),
				'default_value' => '',
				'description'   => esc_html__( 'Set number of columns for portfolio gallery type', 'playerx-core' ),
				'parent'        => $gallery_type_meta_container,
				'options'       => playerx_edge_get_number_of_columns_array( true, array( 'one', 'five', 'six' ) )
			)
		);
		
		playerx_edge_create_meta_box_field(
			array(
				'name'          => 'edgtf_portfolio_single_gallery_space_between_items_meta',
				'type'          => 'select',
				'label'         => esc_html__( 'Space Between Items', 'playerx-core' ),
				'description'   => esc_html__( 'Set space size between columns for portfolio gallery type', 'playerx-core' ),
				'default_value' => '',
				'options'       => playerx_edge_get_space_between_items_array( true ),
				'parent'        => $gallery_type_meta_container
			)
		);
		
		/***************** Gallery Layout *****************/
		
		/***************** Masonry Layout *****************/
		
		$masonry_type_meta_container = playerx_edge_add_admin_container(
			array(
				'parent'          => $meta_box,
				'name'            => 'edgtf_masonry_type_meta_container',
				'dependency' => array(
					'show' => array(
						'edgtf_portfolio_single_template_meta'  => array(
							'masonry',
							'small-masonry'
						)
					)
				)
			)
		);
		
		playerx_edge_create_meta_box_field(
			array(
				'name'          => 'edgtf_portfolio_single_masonry_columns_number_meta',
				'type'          => 'select',
				'label'         => esc_html__( 'Number of Columns', 'playerx-core' ),
				'default_value' => '',
				'description'   => esc_html__( 'Set number of columns for portfolio masonry type', 'playerx-core' ),
				'parent'        => $masonry_type_meta_container,
				'options'       => playerx_edge_get_number_of_columns_array( true, array( 'one', 'five', 'six' ) )
			)
		);
		
		playerx_edge_create_meta_box_field(
			array(
				'name'          => 'edgtf_portfolio_single_masonry_space_between_items_meta',
				'type'          => 'select',
				'label'         => esc_html__( 'Space Between Items', 'playerx-core' ),
				'description'   => esc_html__( 'Set space size between columns for portfolio masonry type', 'playerx-core' ),
				'default_value' => '',
				'options'       => playerx_edge_get_space_between_items_array( true ),
				'parent'        => $masonry_type_meta_container
			)
		);
		
		/***************** Masonry Layout *****************/
		
		playerx_edge_create_meta_box_field(
			array(
				'name'          => 'edgtf_show_title_area_portfolio_single_meta',
				'type'          => 'select',
				'default_value' => '',
				'label'         => esc_html__( 'Show Title Area', 'playerx-core' ),
				'description'   => esc_html__( 'Enabling this option will show title area on your single portfolio page', 'playerx-core' ),
				'parent'        => $meta_box,
				'options'       => playerx_edge_get_yes_no_select_array()
			)
		);
		
		playerx_edge_create_meta_box_field(
			array(
				'name'        => 'portfolio_info_top_padding',
				'type'        => 'text',
				'label'       => esc_html__( 'Portfolio Info Top Padding', 'playerx-core' ),
				'description' => esc_html__( 'Set top padding for portfolio info elements holder. This option works only for Portfolio Images, Slider, Gallery and Masonry portfolio types', 'playerx-core' ),
				'parent'      => $meta_box,
				'args'        => array(
					'col_width' => 3,
					'suffix'    => 'px'
				)
			)
		);
		
		playerx_edge_create_meta_box_field(
			array(
				'name'        => 'portfolio_external_link',
				'type'        => 'text',
				'label'       => esc_html__( 'Portfolio External Link', 'playerx-core' ),
				'description' => esc_html__( 'Enter URL to link from Portfolio List page', 'playerx-core' ),
				'parent'      => $meta_box,
				'args'        => array(
					'col_width' => 3
				)
			)
		);
		
		playerx_edge_create_meta_box_field(
			array(
				'name'        => 'edgtf_portfolio_featured_image_meta',
				'type'        => 'image',
				'label'       => esc_html__( 'Featured Image', 'playerx-core' ),
				'description' => esc_html__( 'Choose an image for Portfolio Lists shortcode where Hover Type option is Switch Featured Images', 'playerx-core' ),
				'parent'      => $meta_box
			)
		);
		
		playerx_edge_create_meta_box_field(
			array(
				'name'          => 'edgtf_portfolio_masonry_fixed_dimensions_meta',
				'type'          => 'select',
				'label'         => esc_html__( 'Dimensions for Masonry - Image Fixed Proportion', 'playerx-core' ),
				'description'   => esc_html__( 'Choose image layout when it appears in Masonry type portfolio lists where image proportion is fixed', 'playerx-core' ),
				'default_value' => '',
				'parent'        => $meta_box,
				'options'       => array(
					''                   => esc_html__( 'Default', 'playerx-core' ),
					'small'              => esc_html__( 'Small', 'playerx-core' ),
					'large-width'        => esc_html__( 'Large Width', 'playerx-core' ),
					'large-height'       => esc_html__( 'Large Height', 'playerx-core' ),
					'large-width-height' => esc_html__( 'Large Width/Height', 'playerx-core' )
				)
			)
		);
		
		playerx_edge_create_meta_box_field(
			array(
				'name'          => 'edgtf_portfolio_masonry_original_dimensions_meta',
				'type'          => 'select',
				'label'         => esc_html__( 'Dimensions for Masonry - Image Original Proportion', 'playerx-core' ),
				'description'   => esc_html__( 'Choose image layout when it appears in Masonry type portfolio lists where image proportion is original', 'playerx-core' ),
				'default_value' => '',
				'parent'        => $meta_box,
				'options'       => array(
					''            => esc_html__( 'Default', 'playerx-core' ),
					'large-width' => esc_html__( 'Large Width', 'playerx-core' )
				)
			)
		);
	}
	
	add_action( 'playerx_edge_action_meta_boxes_map', 'playerx_core_map_portfolio_settings_meta', 41 );
}